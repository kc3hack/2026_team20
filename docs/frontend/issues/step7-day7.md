...
- [ ] トースト通知が成功/失敗時に表示される
- [ ] **リアルタイム同期が動作する** — 2 ブラウザで同じ Plot を開き、片方の編集が他方にリアルタイム反映されること
- [ ] **セクションロックが正常に動作する** — Y.js Awareness で「🔒 ○○が編集中」が表示され、同時に同じセクションを編集できないこと
- [ ] **ロック自動解放** — 編集者がタブを閉じた場合、30 秒以内にロックが解除されること
