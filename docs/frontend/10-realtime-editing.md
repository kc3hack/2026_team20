...
stateDiagram-v2
    ...
    Unlocked --> LockedByMe : 編集ボタンクリック（Awareness に editingSectionId をセット）
    Unlocked --> LockedByOther : 他ユーザーの Awareness が editingSectionId をセット

    LockedByMe --> Unlocked : 編集完了 / ページ離脱（Awareness クリア / 自動タイムアウト）
    LockedByMe --> LockedByMe : Y.js Awareness が自動でハートビート

    LockedByOther --> Unlocked : 他ユーザーの Awareness が消える（編集完了 or 切断）
    LockedByOther --> LockedByOther : リアルタイム内容同期（閲覧のみ）
```
...
```mermaid
sequenceDiagram
    participant UserA as ユーザー A（編集者）
    participant Frontend as フロントエンド
    participant Yjs as Y.js Awareness
    participant RT as Supabase Realtime
    participant UserB as ユーザー B（閲覧者）

    UserA->>Frontend: 「編集する」ボタンクリック
    Frontend->>Frontend: Awareness に他ユーザーが同じセクションを編集中か確認
    alt セクションは空いている
        Frontend->>Yjs: awareness.setLocalState({ editingSectionId: sectionId, user })
        Yjs->>RT: Awareness 状態を Broadcast
        RT-->>UserB: Awareness 更新通知
        UserB->>UserB: 「🔒 ユーザーA が編集中」表示
    else セクションは他ユーザーが編集中
        Frontend->>Frontend: toast.error("このセクションは ○○ が編集中です")
    end

    loop 編集中
        UserA->>Frontend: テキスト入力
        Frontend->>RT: Y.js 差分を Broadcast
        RT-->>UserB: Y.js 差分を受信
        UserB->>UserB: リアルタイムで内容が更新される（閲覧のみ）
    end

    UserA->>Frontend: 「編集完了」or ページ離脱
    Frontend->>Yjs: awareness.setLocalState({ editingSectionId: null, user })
    Frontend->>Frontend: PUT /sections/{id} で最終コンテンツ保存
    Yjs->>RT: Awareness 状態を Broadcast
    RT-->>UserB: Awareness 更新通知
    UserB->>UserB: ロック解除表示、最新内容で表示更新
```
...
```mermaid
flowchart TD
    A["ページ表示"] --> B["usePlotDetail(id) でPlotデータ取得"]
    A["ページ表示"] --> B["usePlotDetail(id) でPlotデータ取得"]
    B --> C["plot:{plotId} チャネルで Y.js Awareness を購読"]
    C --> D["Awareness 変更を監視 → 各セクションの lockState を更新"]
    D --> E["接続完了まで各セクションは「unknown」状態"]

    E --> F{"セクション N のロック状態?"}

    F -->|unlocked| G["SectionViewer + 「編集する」ボタン"]
    F -->|locked-by-me| H["SectionEditor（Tiptap 編集モード）"]
    F -->|locked-by-other| I["SectionViewer（リアルタイム更新）+ SectionLockBadge"]

    G -->|"「編集する」クリック"| J{"ログイン済み?"}
    J -->|No| K["toast: ログインが必要です"]
    J -->|Yes| L["useSectionLock.acquireLock()"]
    L -->|成功| H
    L -->|失敗| M["toast.error: ○○が編集中です"]

    H -->|"「編集完了」クリック"| N["useSectionLock.releaseLock() + PUT /sections/{id}"]
    N --> G
```
...
> - `PlotResponse` に `editingUsers` フィールドは存在しない（API 仕様変更で削除済み）。ロック判定は Awareness のみで行う。初期ロック状態は Awareness 接続待ちとなる（「接続中...」表示）
