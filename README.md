...
![Plot Platform](https://kc3.me/cms/wp-content/uploads/2026/02/444e7120d5cdd74aa75f7a94bf8821a5-scaled.png)
<!-- イメージ画像を差し替えてください -->
...
Plot Platformは、誰でも企画書（Plot）を投稿し、コミュニティの力でリアルタイムに共同編集・改善できるオープンなプラットフォームです。Wikipediaのように誰でも編集でき、GitHubのようにスター・フォークで派生が生まれ、SNSのようにトレンドから面白い企画を発見できます。
...
**Plot Platform**は、Wiki形式の共同編集プラットフォームです。
...
| 機能 | 説明 |
|------|------|
| **リアルタイム共同編集** | セクション単位でリッチテキストを複数人が同時に編集。CRDTによる競合のない同期 |
...
| **画像アップロード** | 企画書に画像を挿入可能（自動リサイズ・サニタイズ付き） |
...
4. **編集に参加** — セクションをクリックしてリアルタイムに共同編集（PC推奨）
...
- **Tiptapリッチテキストエディタ**: 文字色・見出し・リスト・画像挿入など、企画書に必要な表現力を備えつつ、シンプルなUIを維持

### その他

- **CRDTによる競合解決**: Y.jsを採用し、複数人の同時編集でもデータの一貫性を保証。サーバーを介さないP2P的なマージが可能
...
| Y.js + y-prosemirror | 13.x | CRDT（リアルタイム共同編集） |
